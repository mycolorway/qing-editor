...
class Clipboard extends QingModule
  ...
  _setOptions: (opts) ->
    super
    $.extend @opts, Clipboard.opts, opts
  ...
  _processPasteByClipboardApi: (e) ->
    ...
    if e.originalEvent.clipboardData && e.originalEvent.clipboardData.items &&
      ...
      if /^image\//.test(pasteItem.type)
        ...
        return if @editor.trigger('pasting', [imageFile]) == false
  ...
  _getPasteContent: (callback) ->
    @_pasteBin = $ '<div contenteditable="true" />'
      .addClass 'qing-editor-paste-bin'
      .attr 'tabIndex', '-1'
      .appendTo @editor.el
  ...
  _processPasteContent: (pasteContent) ->
    return if @editor.trigger('pasting', [pasteContent]) == false
