...
class TableButton extends Button
  ...
  _init: ->
    ...
    @editor.on 'selectionchanged.table', (e) =>
      @editor.body.find('.qing-editor-table td, .qing-editor-table th')
        .removeClass('active')
      range = @editor.selection.range()
      return unless range
      $container = @editor.selection.containerNode()

      if range.collapsed and $container.is('.qing-editor-table')
  ...
  initResize: ($table) ->
    ...
    $wrapper.on 'mousedown', '.qing-editor-resize-handle', (e) ->
      ...
      $(document).on 'mousemove.qing-editor-resize-table', (e) ->
      ...
      $(document).one 'mouseup.qing-editor-resize-table', (e) ->
  ...
  undecorate: ($table) ->
    return unless $table.parent('.qing-editor-table').length > 0
